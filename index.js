...
function onIntent(intentRequest, session, callback) {
    ...
    else if ("EmailFromIntent" == intentName) {
        handleFromEmail(intent,session,callback);
    }
    else if ("EmailToIntent" == intentName) {
        handleToEmail(intent,session,callback);

    }
    else if ("EmailOnIntent" == intentName) {
        handleOnEmail(intent,session,callback);
    }
    else if ("EmailDateRangeIntent" == intentName) {
        handleDateRangeEmail(intent,session,callback);
    }
...
function parseEmailOnObject(data)
{
    var speechOutput = "";
    var speechOutput1 = "";
    var speechOutput2 = "";
    ...
    for(var i = 0; i < emailList.length; i++){
        count = count + 1;
        speechOutput2 = speechOutput2.concat("Message from " + emailList[i]['from'].split('<')[0] +" sent on "+emailList[i]['date']+" with subject "+ emailList[i]['subject']+". ")

    }
    // data.Items.forEach(function(email) {
    //     count = count + 1;
    //     speechOutput2 = speechOutput2.concat("Message from " + email['from']+" at "+email['date']+" with subject "+ email['subject']+". ")
    speechOutput1 = "You have "+emailList.length+" emails. Let me read them out. ";
    speechOutput1 = speechOutput1.concat(speechOutput2);
    console.log(speechOutput);
    return speechOutput1;
}
...
function handleUnreadEmail(intent, session, callback) {
    ...
    gmailJSON(vdata, function (data) {
        ...
        if (data != "Empty") {
            ...
            speechOutput = parseEmailOnObject(JSON.parse(data));
        } else {
            speechOutput = "You have no such emails matching this search criteria";
            console.log("You have no such emails matching this search criteria");
        }
        callback(session.attributes, buildSpeechletResponse("some_Header", speechOutput, "some_Reprompt", shouldEndSession));

    });
}

function handleOnEmail(intent, session, callback) {
    var speechOutput = "";
    var ds = intent.slots.OnDate.value;
    console.log(ds);
    console.log("On Date");
    var vdata = '{"on" :"'+ds+'"}';
    var post_data = JSON.stringify(vdata);
...
function handleFromEmail(intent, session, callback){
    var speechOutput = "";
    var vdata = '{"sender" : "mrnaik@scu.edu"}';
    // Build the post string from an object
    var post_data = JSON.stringify(vdata);
    console.log("Post_data:" + vdata);
...
function handleToEmail(intent, session, callback){
    var speechOutput = "";
    var vdata = '{"to" : "mrnaik@scu.edu"}';
    // Build the post string from an object
    var post_data = JSON.stringify(vdata);
    console.log("Post_data:" + vdata);
...
function handleDateRangeEmail(intent,session,callback) {
    var speechOutput = "";
    //console.log("Slot value:"+slots.OnDate.value);
    var sd = intent.slots.StartDate.value;
    var ed = intent.slots.EndDate.value;
    console.log(sd);
    console.log(ed);
    ...
    //     month = ds.split("-")[1],
    //     date = ds.split("-")[2];
    // var dd = 'datetime.date('+year+','+month+','+date+')';
    // console.log(dd);
    var vdata = '{"after" :"'+sd+'"+"before" :"'+ed +'"}';
    // Build the post string from an object
    var post_data = JSON.stringify(vdata);
    console.log("Post_data:" + vdata);
    gmailJSON(vdata, function (data) {
...
function handleMedicineDosageRequest(intent, session, callback) {
    ...
    if(intent.slots.Medicine.value !== undefined && intent.slots.ExpiryDate.value !== undefined && intent.slots.Instruction.value !== undefined) {
        ...
        var speechOutput1 = "You have to specify medicine name, schedule expiry date, instruction and time of the day. "+
            "You can say something like,  record two pills of Diabeta after meal everyday in the afternoon until July 31st"+
...
function handleFinishSessionRequest(intent, session, callback) {
    ...
    callback(session.attributes,
        buildSpeechletResponseWithoutCard("Happy to Help you if you need anything else! Good bye!", "", true));
}
